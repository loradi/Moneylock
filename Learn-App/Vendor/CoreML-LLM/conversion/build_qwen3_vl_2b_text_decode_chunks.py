"""Qwen3-VL 2B text-decode converter — ANE-OPTIMIZED + fused MLP.

Ports the VL4B ANE recipe to the 2B model. 2B is ~48% of 4B's params
(28 layers / hidden=2048 / intermediate=6144 vs 4B's 36 / 2560 / 9728),
so every per-step op is proportionally faster, and chunks stay well
under iPhone ANE's single-mlprogram compile envelope at max_seq=2048.

Applies the Gemma 4 ANE recipe (proven 11→31 tok/s on gemma4e2b):
...
Output paths land under `qwen3_vl_2b_decode_chunks/` — Swift loader
has a parallel path to the 4B layout, but 2B's own folder keeps the
two from colliding when both are sideloaded during bench.
"""
...
# 28-layer / hidden=2048 / intermediate=6144 / vocab=151936 / GQA 2:1
# (num_heads=16, num_kv_heads=8, head_dim=128). Lighter than 4B on
# every axis, so max_seq=2048 chunks fit iPhone ANE comfortably.
MAX_SEQ = 2048
NUM_BODY_CHUNKS = 4   # 28 / 4 = 7 layers per chunk
LAYERS_PER_CHUNK = 7
...
class ANEDecoderLayer(nn.Module):
    ...
    def __init__(self, cfg, hf_layer, max_seq):
        ...
        # Fused gate_up MLP projection: stack gate_proj + up_proj weights
        # into a single Conv2d so ANE does one matmul instead of two.
        # The two originals both consume the same `x`, so this is
        # mathematically identical.
        gate_w = hf_layer.mlp.gate_proj.weight   # (intermediate, hidden)
        up_w = hf_layer.mlp.up_proj.weight
        intermediate = gate_w.shape[0]
        stacked = torch.cat([gate_w, up_w], dim=0)  # (2*intermediate, hidden)
        self.gate_up_proj = Conv2dLinear(
            gate_w.shape[1], 2 * intermediate,
    ...
    def forward(self, hidden_conv, position, cos, sin, k_cache, v_cache):
        ...
        # Fused gate_up: 1 Conv2d dispatch instead of 2
        gate_up = self.gate_up_proj.forward_conv(h_conv)  # (1, 2*I, 1, 1)
