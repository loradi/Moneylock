"""Qwen3-VL 4B text-decode converter — ANE + fused SDPA variant.

Extends the ANE recipe with a fused `F.scaled_dot_product_attention`
call so CoreML emits the ANE-native `mb.scaled_dot_product_attention`
op (iOS18+). Replaces the hand-rolled matmul/softmax/matmul path with
a single fused op the ANE compiler can specialize.

Full recipe:
  1. **Conv2dLinear** for ALL projections (q/k/v/o/gate/up/down/lm_head)
  2. **ANERMSNorm** for all RMSNorm sites
  3. **repeat_kv_ane** for GQA KV repeat
  4. **F.scaled_dot_product_attention** replaces manual attention core
  5. **InModelArgmax** in chunk_head
...
class ANEDecoderLayer(nn.Module):
    ...
    def forward(self, hidden_conv, position, cos, sin, k_cache, v_cache):
        ...
        # Fused SDPA: ANE has a native scaled_dot_product_attention op
        # (mb.scaled_dot_product_attention, iOS18+). Pass a pre-built
        # additive causal mask for positions > current.
        # q: (1, H, 1, D); k_rep/v_rep: (1, H, max_seq, D)
        causal_mask = (self.positions.view(1, 1, 1, self.max_seq) > pos) \
            .to(q.dtype) * -1e4
